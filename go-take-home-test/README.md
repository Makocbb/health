...
Go port of the Healthtech-1 take-home starter (`take-home-test`).
...
- Conform to the schema we've currently agreed with the external provider. This schema is found in `internal/forms/schemas/ingested.go` (but unfortunately the data source isn't 100% reliable and schema changes aren't always communicated in a timely fashion!)
...
- Be transformed into the schema found in `internal/forms/schemas/transformed.go`.
...
- This starter uses [Echo](https://echo.labstack.com/) for HTTP
